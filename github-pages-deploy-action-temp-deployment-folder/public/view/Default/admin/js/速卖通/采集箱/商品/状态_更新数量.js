var fun =
{
    obj: { A1: 1, A2: 70, Aarr: [] },
    a01: function () {
        let html = Tool.header('正在【状态_更新数量】任务...') + '\
        <div class="p-2">\
    ...
    a03: function () {
        let html = '<.Db(sqlite.aliexpress,select count(1) as total from @.pro where @.hide=' + (this.obj.A1 - 1) + ',count)/>'
        $("#state").html("正在统计数量");
    ...
    a05: function () {
        config.pro_hide_count = this.obj.Aarr;
        let str = '<r: file="/' + o.path + 'admin/js/速卖通/采集箱/商品/config.js">let config=' + JSON.stringify(config, null, 2) + '</r:>';
