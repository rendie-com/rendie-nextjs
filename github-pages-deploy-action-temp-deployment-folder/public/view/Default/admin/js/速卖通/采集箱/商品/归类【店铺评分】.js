...
var fun =
{
    obj: { A1: 1, A2: 0 },
    a01: function () {
        let html = Tool.header('正在归类【店铺评分&lt;96】...') + '\
    <div class="p-2">\
    ...
    a02: function () {
		...
		<r:shop db="sqlite.aliexpress" size=50 page=2 where=" where @.score<96">,<:shopid/></r:shop>]'
        $("#state").html("正在获取店铺ID...");
    ...
    a04: function (oo) {
        oo.shift()
        let str = '""<r: db="sqlite.aliexpress">update @.pro set @.hide=41,@.err=\'归类【店铺评分<96】\' where @.shopid in(' + oo.join(",") + ')</r:>'
