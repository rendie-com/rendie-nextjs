...
var fun =
{
  a01: function () {
    ...
    let path = "admin/js/速卖通/品牌/"
    switch (obj.arr[3]) {
      case "js01": Tool.scriptArr([path + '获取商品表中的品牌.js']); break;
      case "js02": Tool.scriptArr([path + '从品牌名称中获取信息.js']); break;
      default:
  ...
  a02: function () {
    ...
    "count":<@count/>\
		...
		<r:brand db="sqlite.aliexpress" page=2 size=20 '+ (obj.arr[5] == '-_-20' ? '' : 'where=" where @.name like\'%' + Tool.unescape(obj.arr[5]) + '%\'"') + '>,\
    {\
			"id":<:id/>,\
			"name":<:name tag=json/>,\
			"nameCn":<:nameCn tag=json/>,\
  ...
  a03: function (arr) {
    ...
    html2 += '<tr><td colspan="7">' + Tool.page(arr[0].count, arr[0].size, 4) + '</td></tr>'
    let html = '\
    <header class="panel-heading">【速卖通】品牌</header>\
    <div class="p-2">\
      '+ this.b02() + '\
  ...
  b01: function () {
    return '\
      ...
      <ul class="dropdown-menu">\
        <li onClick="Tool.open4(\'js01\')"><a class="dropdown-item pointer">获取商品表中的品牌</a></li>\
        <li onClick="Tool.open4(\'js02\')" title="速卖通有个查询品牌的地方，是从那里获取的。"><a class="dropdown-item pointer">*从品牌名称中获取信息</a></li>\
      </ul>'
  },
  b02: function () {
    return '\
