...
var fun=
{
  ...
  a04:function(html)
	{
			...
			<div class="p-2">\
				...
				<tbody>\
					...
					<tr>\
						...
						<td><textarea id="typeA" rows="3" class="form-control">'+ this.obj.code.b.listA + '</textarea></td>\
						<td><textarea id="typeB" rows="3" class="form-control">'+ this.obj.code.b.listB +'</textarea></td>\
					</tr>\
					<tr><td class="w150  right">出错说明：</td><td colspan="2">获取【列表链接】不正确</td></tr>\
					<tr>\
						<td class="right">出错内容：</td>\
						<td colspan="2"><textarea  rows="12" class="form-control" id="viewsource">'+html+'</textarea></td>\
					</tr>\
					<tr>\
						<td></td>\
						<td colspan="2">\
							<button type="button" class="btn btn-outline-secondary" onclick="fun.c02();">上一步</button>\
							<button type="button" class="btn btn-outline-secondary" onclick="fun.a09()">提交源码</button>\
						</td>\
					</tr>\
				</tbody>\
				</table>\
		...
		{
			txt = Tool.StrSlice(txt, this.obj.code.b.proidA, this.obj.code.b.proidB);
			if (txt === false)
			{
				alert("截取 链接开始~数据链接结束 失败")
			}
			else {
				if(this.obj.code.b.isspecialmlink=="1"){txt=this.obj.code.b.mlinkRR.replace("[变量]",txt)}//特殊链接处理
				this.obj.url2 = txt;
      ...
      <table class="table table-hover align-middle mb-0">\
        <tbody>\
				...
				<tr>\
					...
					<td><textarea id="htmlcode" class="form-control" wrap="off" rows="12" readonly>'+htmlcode+'</textarea></td>\
				</tr>\
				</tbody>\
      </table>\
			...
			<table lable="1" class="table table-hover align-middle">'+this.b03(codeC)+'</table>\
			<table lable="2" class="table table-hover align-middle" style="display: none;">'+this.b05(codeC)+'</table>\
			<table lable="3" class="table table-hover align-middle" style="display: none;">'+this.b06(codeC)+'</table>\
			<table lable="4" class="table table-hover align-middle" style="display: none;">'+this.b07(codeC)+'</table>\
			<table lable="5" class="table table-hover align-middle" style="display: none;">'+this.b08(codeC)+'</table>\
			<table lable="6" class="table table-hover align-middle" style="display: none;">'+this.b09(codeC)+'</table>\
			<table lable="7" class="table table-hover align-middle" style="display: none;">'+this.b10(codeC)+'</table>\
		</div>'
  ...
  b01:function(oo)
  {
		let reverse,URL=oo.pageurl1
		if(oo.pageset=="1"||oo.pageset=="3")
		{
			URL=URL.replace(/\{\$ID\}/,oo.istart);
		}
		else if(oo.pageset=="2")
		{
			let arr=(oo.pageurl2).split("\n")
			if(oo.reverse=="0"){reverse=0}else{reverse=arr.length-1}
			URL=arr[reverse];
		}
		else if(oo.pageset=="2"){alert("【提交分页】还没做");}
		return URL
  },
  b02:function()
  {
  ...
  b03:function(codeC)
  {
		...
		<tr>\
			<th class="w150"></th>\
			<th class="w-50">截取开始位置：</th>\
			<th>截取结束位置：</th>\
		</th>\
		</thead>\
		<tr>\
			<td class="right">分类ID：</td>\
		...
		<tr>\
			...
			<td><textarea id="err404" rows="3" class="form-control">'+codeC.err404+'</textarea></td>\
  ...
  b07:function(codeC)
  {
		return '\
		<thead class="table-light">\
			<tr><th class="w150"></th><th>截取开始位置:</th><th>截取结束位置:</th></tr>\
		</thead>\
		<tr>\
		...
		<tr>\
			...
			<td colspan="2">\
				...
				<div class="form-check form-check-inline">\
					...
					<label class="custom-control-label" for="desmode1">在该页获取</label>\
				</div>\
			</td>\
		</tr>\
		<tr>\
  ...
  b09:function(codeC)
  {
		return '\
		<thead class="table-light">\
			<tr><th class="w150"></th><th class="w-50">截取开始位置:</th><th>截取结束位置:</th></tr>\
		</thead>\
		<tr>\
  ...
  b10:function(codeC)
  {
		...
		<tr>\
			...
			<td><textarea id="shopUrlA" rows="3" class="form-control">'+codeC.shopUrlA+'</textarea></td>\
			<td><textarea id="shopUrlB" rows="3" class="form-control">'+codeC.shopUrlB+'</textarea></td>\
		</tr>\
		<tr>\
			<td class="right">店铺地址特殊处理：</td>\
			<td colspan="2">\
				<div class="form-check form-check-inline">\
					<input type="radio" class="form-check-input" value="0" name="isspecialShopUrl" id="isspecialShopUrl0" onClick="$(\'#ShopUrlTR\').hide();" '+(codeC.isspecialShopUrl=="0"?'checked="checked"':'')+'>\
					<label class="custom-control-label" for="isspecialShopUrl0">不作设置</label>\
				</div>\
				<div class="form-check form-check-inline">\
					<input type="radio" class="form-check-input" value="1" name="isspecialShopUrl" id="isspecialShopUrl1" onClick="$(\'#ShopUrlTR\').show()" '+(codeC.isspecialShopUrl=="1"?'checked="checked"':'')+'>\
					<label class="custom-control-label" for="isspecialShopUrl1">店铺地址替换</label>\
				</div>\
			</td>\
		</tr>\
		<tr id="ShopUrlTR" '+(codeC.isspecialShopUrl=="0"?'style="display:none"':'')+'>\
			<td class="right">店铺地址替换：</td>\
			<td colspan="2"><textarea id="ShopUrlRR" rows="3" class="form-control">'+codeC.ShopUrlRR+'</textarea></td>\
		</tr>\
		<tr>\
  ...
  c02:function()
  {
		...
		codeC.keywordsA=$("#keywordsA").val();codeC.keywordsB=$("#keywordsB").val()//关键词
		codeC.descriptionA=$("#descriptionA").val();codeC.descriptionB=$("#descriptionB").val()//关键词
		...
		codeC.shopUrlA=$("#shopUrlA").val();codeC.shopUrlB=$("#shopUrlB").val()//店铺地址
		...
		codeC.isspecialShopUrl=$("[name='isspecialShopUrl']:checked").val();//店铺地址特殊处理
		codeC.ShopUrlRR=$("#ShopUrlRR").val();//店铺地址要替换的地址
		...
		let html="<r: db=\"sqlite.aliexpress\">update @.gather set @.code='"+(JSON.stringify(this.obj.code)).replace(/'/ig, "''").replace(/\\/ig, "\\\\")+"' where @.id="+obj.arr[4]+"</r:>";  
  ...
  c03:function(t)
	{
		if(t=="")
		{
      Tool.main('/'+obj.arr[0]+'/list/'+obj.arr[2]+'/js04/'+obj.arr[4]+'/1');
    }else{Tool.at(t);}
	}
}
fun.a01();