...
var fun=
{
  ...
  a01:function()
  {
    ...
    { 
      //obj.arr[4]    账号来源ID
      Tool.scriptArr(['admin/js/Cdiscount/Cdiscount账户/修改js']);
    }
    else{this.a02();}
  ...
  a02:function(t)
  {
    let str='[\
    {}\
    <r:APIaccount size=50 page=5 where=" where @.from=\'cdiscount\' order by @.sort asc,@.id asc">,\
    {\
  ...
  {
    let str='\
    <div class="Tul thead">Cdiscount账户（<a href="https://marketplace-registration.cdiscount.com/" target="_blank">https://marketplace-registration.cdiscount.com/</a>）</div>\
    ...
    let fromid=parseInt(Math.random()*999)
		let html='{if "Fun(Db(select top 1 count(1) from @.APIaccount where @.fromid='+fromid+',count))"=="0"}<r: tag="sql">INSERT into @.APIaccount(:from,:sort,:fromid)VALUES(\'cdiscount\',(SELECT count(1)+1 from @.APIaccount where @.from=\'cdiscount\'),'+fromid+')</r:>{else/}重设{/if}'
