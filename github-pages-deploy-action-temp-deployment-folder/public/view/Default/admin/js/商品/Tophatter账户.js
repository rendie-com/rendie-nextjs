...
var fun=
{
  ...
  a01:function()
  {
    ...
    { 
      //obj.arr[4]    账号来源ID
      Tool.scriptArr(['admin/js/Tophatter/Tophatter账户/修改.js']);
    }
    else{this.a02();}
  ...
  a02:function(t)
  {
    let str='[\
    {}\
    <r:APIaccount size=50 page=5 where=" where @.from=\'tophatter\' order by @.sort asc,@.id asc">,\
    {\
  ...
  {
    let str='\
    <div class="Tul thead">Tophatter账户（<a href="https://www.topsellers.cn/category/help/" target="_blank">https://www.topsellers.cn/category/help/</a>）</div>\
    ...
    let fromid=parseInt(Math.random()*999)
		let html='{if "Fun(Db(select top 1 count(1) from @.APIaccount where @.fromid='+fromid+',count))"=="0"}<r: tag="sql">INSERT into @.APIaccount(:from,:sort,:fromid)VALUES(\'tophatter\',(SELECT count(1)+1 from @.APIaccount where @.from=\'tophatter\'),'+fromid+')</r:>{else/}重设{/if}'
