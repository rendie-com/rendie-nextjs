...
var fun=
{
  ...
  a01:function()
	{
		let html='\
    <r:seller db="mysql.Amazon" size=1 where=" where @.fromid='+obj.arr[4]+'">\
    {\
      "UserName":"<:UserName/>",\
      "password":"<:password/>",\
      "name":"<:name/>",\
      "industry":"<:industry/>",\
      "shippingModel":<:shippingModel tag=0/>,\
      "afterSaleID":"<:afterSaleID/>",\
      "Group":<:Group tag=0/>,\
      "token":<:token tag=0/>,\
      "SizeTemplate":<:SizeTemplate tag=0/>,\
      "fromid":<:fromid/>,\
      "upmode":<:upmode tag=0/>\
    }</r:seller>'
  ...
  a02:function(oo)
  {
    this.obj=oo;
		let html='\
		<header class="panel-heading"><a href="javascript:" onclick="Tool.main()" class="arrow_back"></a>Amazon账户_修改</header>\
    <div class="p-2">\
  ...
  c01:function(This,L,V)
  {
		...
		if(val!=V&&!This.attr("disabled"))
		{
			...
			let txt='""<r: db="mysql.Amazon">update @.seller set @.'+L+'=\''+val+'\' where @.fromid='+obj.arr[4]+'</r:>'
