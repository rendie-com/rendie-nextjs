...
var fun =
{
    obj: { A1: 1, A2: 0 },
    a01: function () {
        let html = Tool.header('1688 &gt; 采集箱 &gt; 主商品列表 &gt; 敦煌手动审核状态_获取【敦煌手动审核状态】') + '\
        <div class="p-2">\
    ...
    a04: function (arr) {
        ...
        for (let i = 1; i < arr.length; i++) {
            newArr.push('update @.product set @.ManualReview=' + arr[i].ManualReview + ' where @.proid=\'' + arr[i].proid + '\'')
        }
        Tool.ajax.a01('"ok"<r: db="sqlite.1688">' + newArr.join("<1/>") + '</r:>', 1, this.a05, this)
    },
    a05: function (t) {
        if (t == "ok") {
            this.obj.A1++;
