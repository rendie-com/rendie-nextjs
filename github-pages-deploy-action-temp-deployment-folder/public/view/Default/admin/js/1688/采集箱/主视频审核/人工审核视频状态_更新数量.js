var fun =
{
    ...
    a01: function () {
        let html = Tool.header(obj.params.return, '1688 &gt; 采集箱 &gt; 主商品列表 &gt; 人工审核视频状态_更新数量') + '\
        <div class="p-2">\
    ...
    a03: function () {
        let data = [{
            action: "sqlite",
            database: "1688",
            sql: "select count(1) as total from @.product where @.ManualReview_video_status=" + (this.obj.A1 - 1),
        }]
        $("#state").html("正在统计数量");
