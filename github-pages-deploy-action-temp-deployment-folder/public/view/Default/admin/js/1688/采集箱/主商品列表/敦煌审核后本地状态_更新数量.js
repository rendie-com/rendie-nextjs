var fun =
{
    obj: { A1: 1, A2: 5, Aarr: [] },
    a01: function () {
        let html = Tool.header('1688 &gt; 采集箱 &gt; 主商品列表 &gt; 敦煌审核后本地状态_更新数量') + '\
        <div class="p-2">\
    ...
    a03: function () {
        let html = '<.Db(sqlite.1688,select count(1) as total from @.product where @.DHAfterReview=' + (this.obj.A1 - 1) + ',count)/>'
        $("#state").html("正在统计数量");
    ...
    a05: function () {
        ...
        let str = '"ok"<r: file="/' + o.path + 'admin/js/1688/采集箱/主商品列表/config.js">let config=' + JSON.stringify(config, null, 2) + '</r:>';
