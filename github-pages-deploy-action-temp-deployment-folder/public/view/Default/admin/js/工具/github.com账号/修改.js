...
var fun =
{
  ...
  a02: function () {
    let data = [{
      action: "sqlite",
      database: "tool",
      sql: "select " + Tool.fieldAs("sort,username,password,token,note,name,bakemail") + " FROM @.github where @.id=" + obj.params.id,
    }]
    ...
    let html = Tool.header(obj.params.return, "github.com账号 -&gt; 修改") + '\
        <div class="p-2">\
          <table class="table table-hover align-middle" id="div1">\
            ...
            <tr>\
              <td class="right">备用邮箱：</td>\
              <td><input type="text" class="form-control" value="'+ oo.bakemail + '" onblur="fun.c01($(this),\'bakemail\',\'' + oo.bakemail + '\')"></td>\
              <td></td>\
            </tr>\
            <tr>\
              <td class="right">token：</td>\
              <td><input type="text" class="form-control" value="'+ oo.token + '" onblur="fun.c01($(this),\'token\',\'' + oo.token + '\')"></td>\
              <td></td>\
            </tr>\
            <tr>\
  ...
  c01: function (This, L, V) {
    ...
    if (val != V && !This.attr("disabled")) {
      ...
      let txt = '""<r: db="sqlite.tool">update @.github set @.' + L + '=\'' + val + '\' where @.id=' + obj.arr[4] + '</r:>'
