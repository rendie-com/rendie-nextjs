...
var fun =
{
    ...
    a05: function (oo) {
        if (oo.code == "【1】等待RenDie软件来滑动滑块。") {
            this.d01(827, 694);
        }
        else if (oo.code == "【2】等待RenDie软件来滑动滑块。") {
            this.d01(823, 615);
        }
        else {
    ...
    d01: function (x,y) {
        let oo = {
            clientX: Tool.randomRange(x, x+5), screenY: Tool.randomRange(y, y+5),//鼠标初始位置
            left: 255,//移动距离
            x: 0, y: 0//移动位置
        }
    ...
    d02: function (oo) {
        let sign
        if (oo.x > oo.left) {
            Tool.Time("name", Tool.randomRange(300, 500), this.d04, this, oo);
        }
        else {
            //快到的时后慢点
            if (oo.x > oo.left - 5) {
    ...
    d04: function (oo) {
        ...
        Tool.Time("name", 500, this.d05, this, oo);
    },
    d05: function (oo) {
        win.rightClick1(oo.clientX, oo.screenY, window.screen.width, window.screen.height, this.d06, this)
    },
    d06: function () {
        let txt = '"ok"<r: db="sqlite.tool">update @.action set @.code=null where @.js=\'淘宝滑块验证.js\'</r:>'
        Tool.ajax.a01(txt, 1, this.d07, this);
    },
    d07: function (t) {
        if (t == "ok") {
            Tool.Time("name", 500, this.a04, this);
        }
