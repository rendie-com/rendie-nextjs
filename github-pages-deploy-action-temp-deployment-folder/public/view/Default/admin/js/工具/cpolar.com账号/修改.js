...
var fun =
{
  ...
  a02: function () {
    let data = [{
      action: "sqlite",
      database: "tool",
      sql: "select " + Tool.fieldAs("sort,username,password,token,note,id") + " FROM @.cpolar where @.id=" + obj.params.id,
    }]
    Tool.ajax.a01(data, this.a03, this)
  },
  a03: function (t) {
    let oo = t[0][0]
    let html = Tool.header(obj.params.return, "工具 &gt; cpolar.com账号 -&gt; 修改") + '\
        <div class="p-2">\
  ...
  c01: function (This, L, V) {
    ...
    if (val != V && !This.attr("disabled")) {
      ...
      let txt = '""<r: db="sqlite.tool">update @.cpolar set @.' + L + '=\'' + val + '\' where @.id=' + obj.arr[4] + '</r:>'
