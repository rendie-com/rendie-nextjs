  ...
  if(obj.GlobalI<obj.Global.length)
	...
	else
	{
		if(obj.list.length>0)
		{
			...
			let str='<rendie:area tag="file" file="/webcache/TypeList1688.txt">'+JSON.stringify(obj.list)+'</rendie:area>'
...
{
 let str='<rendie:type where=" where @.from=\'1688\' and @.isleaf=1 order by @.id asc" size=1 page=2><:fromid/></rendie:type>,<@count/>';
...
function collectionTypeAttr001(txt)
{
	...
	if(txt.indexOf(" (502) 错误的网关")==-1)
	{
		...
		for(let i=0;i<obj2.result.total;i++){
			...
			<li>{if \"Fun(Db("+attrselect+",count))\"==\"\"}<rendie:area tag=\"sql\">"+attrinsert+"</rendie:area>"+attrinsert+"---已插入{else/}"+attrselect+"---已存在{/if}</li>\
			<li>{if \"Fun(Db("+Bindselect+",count))\"==\"\"}<rendie:area tag=\"sql\">"+Bindinsert+"</rendie:area>"+Bindinsert+"---已插入{else/}"+Bindselect+"---已存在{/if}</li>\
...
function collectionTypeAttr002(obj2)
{
  ...
  if(obj2.valueIds){
		for(let i=0;i<obj2.values.length;i++)
		{
			...
			html+="\
			<li>{if \"Fun(Db("+Vselect+",count))\"==\"\"}<rendie:area tag=\"sql\">"+Vinsert+"</rendie:area>"+Vinsert+"---已插入{else/}"+Vselect+"---已存在</if><li>\
			<li>{if \"Fun(Db("+Bindselect+",count))\"==\"\"}<rendie:area tag=\"sql\">"+Bindinsert+"</rendie:area>"+Bindinsert+"---已插入{else/}"+Bindselect+"---已存在</if><li>"
		}
  }
  return html
...
function collectionTypeAttr003(i)
{
	...
	{
		$.ajax({type:"POST",url:"exe.html?"+Math.random(),data:{data:escape('<rendie:area tag="file" file="/webcache/TypePage1688.txt">'+obj.page+'</rendie:area>')},success:function(t){
		  obj.page++;obj.t=setTimeout("collectionTypeAttr();",500);
		}});
	}
}
