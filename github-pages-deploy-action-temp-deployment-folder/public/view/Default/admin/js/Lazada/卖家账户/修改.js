...
var fun =
{
    a01: function () {
        let html = '\
        <r:seller db="sqlite.lazada" size=1 where=" where @.id='+ obj.arr[4] + '">\
        {\
          "password":"<:password/>",\
          "email":"<:email/>",\
          "shopname":"<:shopname/>",\
    ...
    a02: function (oo) {
        let html = '\
        <header class="panel-heading"><a href="javascript:" onclick="Tool.main()" class="arrow_back"></a>Lazada  -&gt; 卖家账户 -&gt; 修改</header>\
        <div class="p-2">\
          <table class="table table-hover align-middle" id="div1">\
            ...
            <tr>\
                <td class="right" title="登录后验证是否为当前账号用的">店铺名称：</td>\
                <td><input class="form-control" type="text" value="'+ oo.shopname + '" onblur="fun.c01($(this),\'shopname\',\'' + oo.shopname + '\')"></td>\
                <td></td>\
            </tr>\
           <tr>\
    ...
    c01: function (This, L, V) {
        ...
        if (val != V && !This.attr("disabled")) {
            ...
            let txt = '""<r: db="sqlite.lazada">update @.seller set @.' + L + '=\'' + val + '\' where @.id=' + obj.arr[4] + '</r:>'
