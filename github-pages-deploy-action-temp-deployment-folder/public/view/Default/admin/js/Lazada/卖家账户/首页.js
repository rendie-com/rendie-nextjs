...
var fun =
{
    ...
    a02: function () {
        let str = '[<@count/>\
        <r:seller db="sqlite.lazada" page=2 size=20>,\
        {\
    ...
    a03: function (arr) {
        ...
        let urlArr = [
            "https://www.lazada-seller.cn/",
        ]
        html = '\
        <header class="panel-heading">Lazada &gt; 卖家账户</header>\
        <div class="p-2">\
    ...
    c01: function () {
        let html = "\"ok\"<r: db=\"sqlite.lazada\">insert into @.seller(@.addtime)values(" + Tool.gettime("") + ")</r:>"
        Tool.ajax.a01(html, 1, Tool.reload)
    },
    c02: function (id) {
        if (confirm('确定要删除吗？')) {
            let html = "\"ok\"<r: db=\"sqlite.lazada\">delete from @.seller where @.id=" + id + "</r:>"
            Tool.ajax.a01(html, 1, Tool.reload)
        }
    }
}
