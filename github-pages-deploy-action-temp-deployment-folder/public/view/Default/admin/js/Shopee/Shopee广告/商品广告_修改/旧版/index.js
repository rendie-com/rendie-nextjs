...
var fun =
{
    ...
    a01: function () {
        ...
        obj.arr[8] = obj.arr[8] ? obj.arr[8] : "-_-20";//请选择【修改状态】
        obj.arr[9] = obj.arr[9] ? obj.arr[9] : "-_-20";//请选择【新增关键词时间】
        this.a02()
    },
    a02: function () {
        let str = '{\
    ...
    b08: function () {
        let arr = [
            "@.site='" + obj.arr[4] + "'",
            "@.product_placement='search_product'"
        ];
        if (obj.arr[6] != "-_-20") { arr.push("@.productID='" + obj.arr[6] + "'"); }
        if (obj.arr[7] != "-_-20") { arr.push("@.state='" + obj.arr[7] + "'"); }
        if (obj.arr[8] != "-_-20") { arr.push("@.editStatus=" + obj.arr[8]); }
        if (obj.arr[9] != "-_-20") {
            let time = Tool.gettime("") - 60 * 60 * 24 * Tool.int(obj.arr[9])
    ...
    b11: function () {
        ...
        <ul class="dropdown-menu">\
            <li onClick="Tool.open5(\'js09\',\''+ obj.arr[4] + '\');"><a class="dropdown-item pointer">*增加新的关键词</a></li>\
            <li onClick="Tool.open5(\'js10\',\''+ obj.arr[4] + '\');"><a class="dropdown-item pointer">*停用所有关键词</a></li>\
            <li onClick="Tool.open5(\'js22\',\''+ obj.arr[4] + '\');"><a class="dropdown-item pointer">*把修改好的搜索广告同步有Shopee平台</a></li>\
        </ul>'
    },
            ...
            <tr '+ (key == arr[i].keyword.keyword ? 'style="font-weight:bold;"' : '') + '>\
                ...
                <td class="p-0 w200">\
                    <div class="input-group">\
                      ...
                      '+ (arr[i].recommended_price ? '<span class="input-group-text" title="推荐出价">' + (arr[i].recommended_price * 0.00001).toFixed(2) + '</span>' : '') + '\
                    </div>\
                </td>\
    ...
    b15: function () {
        ...
        <tr>\
            ...
            <td>'+ this.b18(obj.arr[7], 7, config[obj.arr[4]].ads_keyword_state_count) + '</td>\
            <td>'+ this.b19(obj.arr[8], 8, config[obj.arr[4]].ads_keyword_editStatus_count) + '</td>\
            <td>'+ this.b21(obj.arr[9], 9, config[obj.arr[4]].ads_keyword_key_uptime_count) + '</td>\
        <tr>\
        </table>';
    },
    b16: function (key, proid) {
        ...
        <button title="操作" class="menu-button" data-bs-toggle="dropdown" aria-expanded="false"><div></div><div></div><div></div></button>\
        <ul class="dropdown-menu">\
            <li onClick="fun.c10(\''+ key + '\',\'' + proid + '\')"><a class="dropdown-item pointer">设置为【主推关键词】</a></li>\
        </ul>'
    },
