...
Object.assign(Tool, {
    sku://价格
    {
        ...
        a04: function (tier_variation, sku_1688, freight, proid, maxPrice) {
            ...
            if (arr1.length * arr2.length > 100) {
                arr2 = this.b09(arr1.length, arr2)//购物选项不能超过100个(这是shopee设定的)
                tier_variation[1].options = arr2;
            }
        ...
        b06: function (ijArr, val, sku_1688, freight, proid, maxPrice)//一维和二组的post提交json并装
        {
            ...
            if (o1) {
                ...
                oo = {
                    ...
                    is_default: false,
                    mtsku_model_id: 0,
                    seller_sku: proid + "-" + this.b08(ijArr),
        ...
        b09: function (len, arr) {
            let nArr = []
            for (let i = 0; i < arr.length; i++) {
                nArr.push(arr[i]);
                if (len * (nArr.length + 1) > 100) { break; }
            }
            return nArr;
        },
    },
