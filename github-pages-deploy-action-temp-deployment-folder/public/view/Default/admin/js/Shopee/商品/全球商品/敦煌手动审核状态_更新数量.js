var fun =
{
    obj: { A1: 1, A2: 20, Aarr: [] },
    a01: function () {
        let html = Tool.header('Shopee &gt; 商品列表 &gt; 全球商品 &gt; 敦煌手动审核状态_更新数量') + '\
        <div class="p-2">\
    ...
    a03: function () {
        let html = '<.Db(sqlite.shopee,select count(1) as total from @.GlobalPro where @.ManualReview=' + (this.obj.A1 - 1) + ',count)/>'
        $("#state").html("正在统计数量");
    ...
    a05: function () {
        ...
        let str = '""<r: file="/' + o.path + 'admin/js/Shopee/商品列表/全球商品/config.js">let config=' + JSON.stringify(config, null, 2) + '</r:>';
