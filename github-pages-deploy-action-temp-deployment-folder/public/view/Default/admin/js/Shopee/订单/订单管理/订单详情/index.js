...
var fun =
{
    ...
    a02: function () {
        let data = [{
            ...
            sql: "select " + Tool.fieldAs("id,order_id,status,escrow_release_time,order_sn,buyer_address_name,buyer_address_phone,shipping_address,buyer_user,order_items,purchaseInfo,purchasePostingFee,purchaseAccount,purchaseStatus,purchaseNotes,PurchaseRefund,list_type,status_ext,logistics_status,cancel_reason_ext,rate_by_date,auto_cancel_arrange_ship_date,escrow_release_time,payby_date,PurchaseTime,remark") + " FROM @.table where @.order_sn='" + o.params.order_sn + "'",
        }];
    ...
    a04: function (t, oo) {
        ...
        let html = Tool.header(o.params.return, "Shopee &gt; 订单  &gt; 订单详情") + '\
    	<div class="p-2">\
    ...
    b04: function (oo, buyer_user) {
        ...
        <table class="table table-hover align-middle mb-0">\
            ...
            <tr><td class="right">站点：</td><td>'+ Tool.site(o.params.site) + '</td></tr>\
            ...
            <tr><td class="right">买家备注：</td><td>'+ oo.remark + '</td></tr>\
    ...
    b05: function (oo) {
        ...
        if (purchaseNotes) { purchaseNotes = oo.purchaseNotes; } else { purchaseNotes = ""; }
        return '\
        <table class="table table-hover align-middle mb-0">\
            <thead class="table-light"><tr><th colspan="2">1688采购信息</th></tr></thead>\
            <tr><td class="right w150">采购账号：</td><td><input type="text" class="form-control" value="'+ oo.purchaseAccount + '" onblur="fun.c01($(this),\'purchaseAccount\',\'' + oo.purchaseAccount + '\')"></td></tr>\
            ...
            <tr><td class="right">采购贴单费：</td><td><div class="input-group"><input type="text" class="form-control" value="'+ oo.purchasePostingFee + '" onblur="fun.c01($(this),\'purchasePostingFee\',' + oo.purchasePostingFee + ')"><span class="input-group-text" id="inputGroup-sizing-sm">人民币</span></div></td></tr>\
            <tr><td class="right">采购状态：</td><td>'+ this.b06(oo.purchaseStatus) + '</td></tr>\
            <tr><td class="right">采购备注：</td><td><textarea class="form-control h-100" onblur="fun.c01($(this),\'purchaseNotes\')">' + purchaseNotes + '</textarea></td></tr>\
            <tr><td class="right">采购退款金额：</td><td><div class="input-group"><input type="text" class="form-control" value="'+ oo.PurchaseRefund + '" onblur="fun.c01($(this),\'PurchaseRefund\',' + oo.PurchaseRefund + ')"><span class="input-group-text" id="inputGroup-sizing-sm">人民币</span></div></td></tr>\
            <tr><td class="right">采购时间：</td><td>' + Tool.js_date_time2(oo.PurchaseTime) + '</td></tr>\
            <tr>\
                <td colspan="2">\
                <div class="btn-group">\
                    <button id="seep1" type="button" class="btn btn-outline-secondary" onclick="Tool.check_product.a01(\'' + o.params.order_sn + '\',\'' + o.params.site + '\',' + o.params.num + ',\'' + this.obj.siteNum + '\')" title="核对商品">*(1)核对商品</button>\
                    <button id="seep2" type="button" class="btn btn-outline-secondary" onclick="Tool.addCart.a01(\'' + o.params.order_sn + '\',\'' + o.params.site + '\',' + o.params.num + ',\'' + this.obj.siteNum + '\')" title="加入购物车">*(2)加入购物车</button>\
                    <button id="seep3" type="button" class="btn btn-outline-secondary" onclick="Tool.SubmitPayment.a01()" title="提交待付">*(3)提交待付</button>\
                    <button id="seep4" type="button" class="btn btn-outline-secondary" onclick="Tool.get_1688_purchaseInfo.a01(\'' + o.params.order_sn + '\',\'' + this.obj.siteNum + '\')" title="获取采购信息">*(4)获取采购信息</button>\
                    <button id="seep5" type="button" class="btn btn-outline-secondary" onclick="Tool.Arrange_shipment.a01(' + oo.order_id + ',\'' + o.params.site + '\',' + o.params.num + ',\'' + this.obj.siteNum + '\')">*(5)安排出货</button>\
                </div>\
                </td>\
            </tr>\
    ...
    c01: function (This, L, V) {
        ...
        if (val != V && !This.attr("disabled")) {
            ...
            let data = [{
                action: "sqlite",
                database: "shopee/订单/订单管理/" + this.obj.siteNum,
                sql: "update @.table set @." + L + "='" + val + "' where @.order_sn='" + o.params.order_sn + "'",
            }]
    ...
    c03: function (This, L, V) {
        ...
        let data = [{
            action: "sqlite",
            database: "shopee/订单/订单管理/" + this.obj.siteNum,
            sql: "update @.table set @." + L + "=" + V + " where @.order_sn='" + o.params.order_sn + "'",
        }]
        Tool.ajax.a01(data, this.c02, this, This);
    },
    c04: function (This) {
        ...
        if (!This.attr("disabled")) {
            ...
            try {
                ...
                let data = [{
                    action: "sqlite",
                    database: "shopee/订单/订单管理/" + this.obj.siteNum,
                    sql: "update @.table set @.purchaseInfo=" + Tool.rpsql(JSON.stringify(purchaseInfo)) + " where @.order_sn='" + o.params.order_sn + "'",
                }]
                Tool.ajax.a01(data, this.c02, this, This);
            } catch (error) {
                Tool.at("JSON格式不对。")
