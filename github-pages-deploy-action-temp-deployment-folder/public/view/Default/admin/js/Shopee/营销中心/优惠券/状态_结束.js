...
var fun =
{
    ...
    a01: function () {
        ...
        let html = Tool.header("Shopee &gt; 营销中心 &gt; 优惠券 &gt; 状态_结束") + '\
        <div class="p-2">\
        <table class="table table-hover">\
            <tbody>\
                ...
                <tr><td class="right">说明：</td><td colspan="2">【关注礼优惠券】结束不了，是shopee的问题。</td></tr>\
            </tbody>\
        </table>\
        </div>'
    ...
    a04: function () {
        ...
        <r:voucher size=1 db="sqlite.shopee" page=2 where=" where @.fe_status=' + obj.arr[6] + ' and @.site=\'' + obj.arr[5] + '\'">\
            ,<:voucher_id/>\
            ,<:name tag=json/>\
        </r:voucher>]'
    ...
    d02: function (voucher_id) {
        ...
        $("#url").html(url + '【PUT】');
        $("#state").html("正在结束。。。");
        gg.typeHtml(url, "PUT", data, this.d03, this, voucher_id)
    },
    d03: function (t, voucher_id) {
        ...
        //else if (t.message == "ERROR_VOUCHER_NO_EDIT_PERMISSION") {
        //    $("#state").html("结束出错。");
        //    this.d03(voucher_id)
        //}
        else {
    ...
    e01: function (voucher_id) {
        //@.fe_status=3     已过期
        let str = '"ok"<r: db="sqlite.shopee">update @.voucher set @.fe_status=3 where @.voucher_id=' + voucher_id + '</r:>'
