...
var fun =
{
    ...
    a01: function () {
       ...
       // this.obj.A1 = obj.arr[5] ? parseInt(obj.arr[5]) : this.obj.A1;//翻页
        let html = Tool.header("淘宝 &gt; 采集箱 &gt; 给【首图】【放大镜图】【属性图】【详情图】搜货源。。。") + '\
        <div class="p-2">\
    ...
    a06: function (picObj, oo) {
        $("#state").html("正在更新数据...");
        let str = '<r: db="sqlite.taobao">update @.product set @.pic=' + Tool.rpsql(JSON.stringify(picObj)) + ' where @.proid=\'' + oo.proid + '\'</r:>'
    ...
    d04: function (oo) {
        ...
        Tool.ajax.a01('"ok"' + str, 1, this.d05, this, oo)
    },
    d05: function (t, oo) {
        if (t == "ok") {
            $("#state").html("下一步...");
    ...
    e05: function (t, oo) {
        if (t == "ok") {
            ...
            $("#state").html("出错01，延时1秒，上一步...");
            Tool.Time("name", 1000, this.e04, this, oo)
            //Tool.pre(["更新出错03：", t]);
            //console.log(t)
        }
    },
    ...
    f02: function (oo) {
        ...
        else if (Darr.picA.taobao) {
            $("#picD1,#picD2").html("已前处理过了，跳过。");
            this.a08();
        }
        else if (Darr == null || Darr === false) {
            $("#state").html("空位置，过。");
            this.f03(false, oo)
        }
        else {
