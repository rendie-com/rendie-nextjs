...
var fun =
{
    ...
    a01: function () {
        //obj.arr[4]    返回URL
        let html = Tool.header("淘宝 &gt; 采集箱 &gt; 获取敦煌网【手动审核通过】的商品...") + '\
        <div class="p-2">\
    ...
    a04: function (arr) {
        ...
        for (let i = 1; i < arr.length; i++) {
            newArr.push('\
            <if "Fun(Db(sqlite.taobao,select count(1) from @.product where @.proid=\'' + arr[i].proid + '\',count))"=="0">\
                <r: db="sqlite.taobao">Insert into @.product(@.proid,@.pic,@.ManualReview,@.AfterReview,@.addtime) values(\''+ arr[i].proid + '\',' + Tool.rpsql(arr[i].pic) + ',' + arr[i].ManualReview + ',' + arr[i].AfterReview + ',' + Tool.gettime("") + ')</r:>\
            </if>')
        }
