...
$(function(){
				...
				let str="\
					<tr>\
						<td colspan=\"10\" class=\"Mo"+id+"\">\
							<table class=\"tb2\">\
							<rendie:type where=\" where @.from in('admin','user') and @.upid='"+id+"' order by @.sort asc\" size=200>\
							<tr>\
							<td>\
									{if \"<:isleaf/>\"==\"False\"}\
  ...
  $(".makeHtmlTab li").click(function()//显示列表、隐藏列表
  {
    ...
    $("input[name='pre_level']:checked").each(function(){PowerList[PowerList.length]=$(this).val();});
		...
		txt='<rendie:area db="mysql.admin">update @.usergroup set @.PowerList=\''+PowerList.join(",")+'\',:Name=\''+$("#Name").val()+'\',:Descript=\''+$("#Descript").val()+'\' where @.id='+managerID+'</rendie:area>修改成功！'
