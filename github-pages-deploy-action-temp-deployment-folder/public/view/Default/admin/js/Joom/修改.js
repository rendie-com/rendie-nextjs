...
var fun=
{
  ...
  a01:function()
	{
		let html='\
    <r:APIaccount size=1 where=" where @.fromid='+obj.arr[4]+' and @.from=\'joom\'">\
    {\
  ...
  a02:function(oo)
  {
    ...
    if(!oo.upmode.types){oo.upmode.types="";}
		let html='<div class="Tul thead"><a href="javascript:" onclick="Tool.main()" class="arrow_back"></a>修改Joom账户</div>\
  ...
  c01:function(This,L,V)
  {
		...
		if(val!=V&&!This.attr("disabled"))
		{
			...
			let txt="<r: tag=\"sql\">update @.APIaccount set @."+L+"='"+val+"' where @.fromid="+obj.arr[4]+" and @.from='joom'</r:>"
