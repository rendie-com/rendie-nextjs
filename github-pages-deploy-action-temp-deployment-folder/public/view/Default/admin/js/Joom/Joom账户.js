...
var fun =
{
    ...
    a01: function () {
        ...
        {
            //obj.arr[4]    账号来源ID
            Tool.scriptArr(['admin/js/Joom/Joom账户/修改.js']);
        }
        else { this.a02(); }
    ...
    a02: function (t) {
        let str = '[0\
        <r:APIaccount size=50 page=5 where=" where @.from=\'joom\' order by @.sort asc,@.id asc">,\
        {\
    ...
    b01: function (t) {
        let str = '\
        <div class="Tul thead">Joom账户（<a href="https://joom.typeform.com/to/DqosDX" target="_blank">https://joom.typeform.com/to/DqosDX</a>）</div>\
    ...
    c01: function () {
        ...
        let html = '{if "Fun(Db(select top 1 count(1) from @.APIaccount where @.fromid=' + fromid + ',count))"=="0"}<r: tag="sql">INSERT into @.APIaccount(:from,:sort,:fromid)VALUES(\'joom\',(SELECT count(1)+1 from @.APIaccount where @.from=\'joom\'),' + fromid + ')</r:>{else/}重设{/if}'
