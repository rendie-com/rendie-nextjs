...
var fun=
{
  ...
  a02:function() 
  {
    ...
    [\
      {"count":<@count/>,"size":30}\
      <r:dhgatelogpro db="sqlite.main" size=30 page=2 where="'+this.b03()+' order by @.id desc">\
      ,{\
        "proid":"<:proid/>",\
        "proidr":"<:proidr/>",\
        "url":"<:url/>",\
  ...
  a03:function(arr) 
  {
    ...
    for(let i=1;i<arr.length;i++)
    {
      ...
      <tr>\
        ...
        <td class="p-0">\
          <table class="table mb-0 table-bordered table-hover">\
          <tbody>\
            <tr><td title="当前编码">'+arr[i].proid+'</td></tr>\
            <tr><td title="来源编码">'+arr[i].proidr+'</td></tr>\
    ...
    <header class="panel-heading">\
      ...
      <div onclick="Tool.main(\'js01\')" class="active">商品访问日志</div>\
  ...
  b01:function() 
  {
    ...
    <tr>\
      <th class="left" style="padding-left: 30px;position: relative;">\
        ...
        <ul class="dropdown-menu">\
          <li onClick="Tool.open5(\'js04\',\'dhgatelogpro\')"><a class="dropdown-item pointer">*从IP中获取【国家】【归属地】信息</a></li>\
          <li onClick="Tool.open5(\'js03\',\'dhgatelogpro\')"><a class="dropdown-item pointer">从【客户端信息】中获取【系统信息】</a></li>\
  ...
  c04:function(ip)
  {
    let html="delete from @.dhgatelogpro where @.ip='"+ip+"'";
    html='""<r: db="mysql.admin">'+html+'</r:>';
      Tool.ajax.a01(html,1,Tool.reload)
  },
  c05:function()
  {
    let html="delete from @.dhgatelogpro where @.asn like '%云%' or @.asn like '%数据中心%'";//
    html='""<r: db="mysql.admin">'+html+'</r:>';
      Tool.ajax.a01(html,1,Tool.reload)
  },
  c06:function()
  {
    let html="delete from @.dhgatelogpro where @.lang like'%zh%'";
    html='""<r: db="mysql.admin">'+html+'</r:>';
      Tool.ajax.a01(html,1,Tool.reload)
	},
  c07:function()
  {
    let html="delete from @.dhgatelogpro where @.lang=''";
    html='""<r: db="mysql.admin">'+html+'</r:>';
      Tool.ajax.a01(html,1,Tool.reload)
	},
  c08:function()
  {
    let html="delete from @.dhgatelogpro where @.ct='中国'";
    html='""<r: db="mysql.admin">'+html+'</r:>';
      Tool.ajax.a01(html,1,Tool.reload)
	}
}
