...
var fun =
{
        ...
        <tbody>\
				...
				<tr>\
					...
					<div class="form-check form-check-inline">\
						...
						<label class="form-check-label" for="radio0">固定分类</label>\
					</div>\
					<div class="form-check form-check-inline">\
						...
						<label class="form-check-label" for="radio1">智能归类</label>\
					</div>\
					<div class="form-check form-check-inline">\
						...
						<label class="form-check-label" for="radio2">采集分类ID</label>\
					</div>\
					</td>\
				</tr>\
				<tr>\
          ...
          <label class="form-check-label" for="pageset0">不分页</label>\
        </div>\
        <div class="form-check form-check-inline">\
          ...
          <label class="form-check-label" for="pageset1">批量分页</label>\
        </div>\
        <div class="form-check form-check-inline">\
          ...
          <label class="form-check-label" for="pageset2">手动分页</label>\
        </div>\
        <div class="form-check form-check-inline">\
          ...
          <label class="form-check-label" for="pageset3">直接采集内容页(不分页)</label>\
        </div>\
        <div class="form-check form-check-inline">\
          ...
          <label class="form-check-label" for="pageset4">提交分页(post)</label>\
        </div>\
					</td>\
				</tr>\
				<tr style="display:none">\
					<td class="right">内容页链接：</td>\
					<td>\
					<textarea id="pageurlarticle" rows="17" style="width:99%;white-space:nowrap;overflow:scroll;">'+ codeA.pageurlarticle + '</textarea><br/>每行一个链接</font></td>\
				</tr>\
				<tr id="page_1" '+ (codeA.pageset == "1" ? '' : 'style="display:none"') + '>\
					<td class="right">列表页链接：</td>\
					<td class="p-0">\
						<table class="table mb-0 align-middle">\
						<tr>\
							<td class="w150 right">列表页第一页：</td>\
							<td><input type="text" id="pageurl1" value="'+ codeA.pageurl1 + '" class="form-control"/></td>\
							<td class="w100"></td>\
						</tr>\
						<tr>\
							<td class="right">列表页URL：</li>\
							<td><input type="text" id="pageurl" value="'+ codeA.pageurl + '" class="form-control"/></td>\
							<td>变量<font color="red">{$ID}</font></td>\
						</tr>\
						</table>\
						</td>\
				</tr>\
				<tr id="page_4" class="Tul" '+ (codeA.pageset == "4" ? '' : 'style="display:none"') + '>\
				...
				<tr>\
					...
					<td>\
						...
						<td>结束：</td>\
						<td class="w60"><input type="text" id="iend" class="form-control center" value="'+ codeA.iend + '" class="center"/></td>\
						<td>步长：</td>\
						<td class="w60"><input type="text" id="step" class="form-control center" value="'+ codeA.step + '" class="center"/></td>\
				...
				<tr id="page_2" class="Tul" '+ (codeA.pageset == "2" ? '' : 'style="display:none"') + '>\
					<td class="right">手动分页：</td>\
					<td>\
						<textarea id="pageurl2" rows="17" style="width:99%;white-space:nowrap;overflow:scroll;">'+ codeA.pageurl2 + '</textarea>&nbsp;每行一个链接\
					</td>\
				</tr>\
				<tr>\
					<td></td>\
    ...
    c01: function () {
        ...
        codeA.pageurl = $("#pageurl").val()//列表页第一页
        ...
        let str = '""<r: db="sqlite.dhgate">update @.gather set @.sort=' + $("#sort").val() + ',@.name=\'' + ($("#name").val()).replace(/'/ig, "''") + '\',@.note=\'' + ($("#note").val()).replace(/'/ig, "''") + '\',@.code=\'' + (JSON.stringify(this.obj.code)).replace(/'/ig, "''").replace(/\\/ig, "\\\\") + '\' where @.id=' + obj.arr[4] + '</r:>'
        Tool.ajax.a01(str, 1, this.c02, this)
    },
    c02: function (txt) {
        if (txt == "") {
            Tool.main('/' + obj.arr[0] + '/list/' + obj.arr[2] + '/js02/' + obj.arr[4]);
        } else { Tool.at(txt); }
    }
}
fun.a01();