var fun =
{
    obj: { A1: 1, A2: 10, Aarr: [] },
    a01: function () {
        let html = Tool.header('正在【敦煌状态_更新数量】任务...') + '\
        <div class="p-2">\
    ...
    a03: function () {
        let html = '<.Db(sqlite.dhgate,select count(1) as total from @.proupdhgate where @.status=' + (this.obj.A1 - 1) + ',count)/>'
        $("#state").html("正在统计数量");
    ...
    a05: function () {
        config.proupdhgate_status_count = this.obj.Aarr;
