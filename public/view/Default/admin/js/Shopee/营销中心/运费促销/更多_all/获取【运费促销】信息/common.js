    ...
    voucher_list: {
        a01: function (seller, site, num, progress, endTime, next, This, t) {
            let oo = {
                ...
                A1: 1, A2: 0,
                logistics_promotions: [],
            }
            this.a02(oo);
        },
        a02: function (oo) {
            Tool.download_sqlite.a01(["shopee/营销中心/运费促销/" + oo.siteNum], this.a03, this, oo);
        },
        a03: function (t, oo) {
            let arr = [
            ...
            let url = "https://seller.shopee.cn/api/v4/cbsc/logistics_promotion/get_logistics_promotion_list/?" + arr.join("&")
        ...
        a04: function (t, oo) {
            if (t.error == 0) {
                oo.A2 = Math.ceil(t.data.total_num / 10);
                oo.logistics_promotions = t.data.logistics_promotions
                this.d01(oo)
            }
            else if (t.error_msg == "feature toggled off") {
                $("#state").html("功能已关闭");
                this.e01(oo);
            }
            else {
        ...
        b02: function (oo) {
            ...
            for (let k in oo) {
                ...
                arrR.push(oo[k]);
                if (k != "group_rule_id") { updateArr.push("@." + k + "=" + oo[k]); }
            }
        ...
        d02: function (oo) {
            let arr = oo.logistics_promotions;
            if (oo.endTime != 0 && arr[0].end_time < oo.endTime) {
                ...
                for (let i = 0; i < arr.length; i++) {
                    ...
                    data.push({
                        action: "sqlite",
                        database: "shopee/营销中心/运费促销/" + oo.siteNum,
                        sql: "select @.id from @.table where @.group_rule_id=" + arr[i].group_rule_id,
                        list: [{
                            action: "sqlite",
                            database: "shopee/营销中心/运费促销/" + oo.siteNum,
                            sql: "update @.table set " + arrLRU[2].join(",") + "  where  @.group_rule_id=" + arr[i].group_rule_id,
                        }],
                        elselist: [{
                            action: "sqlite",
                            database: "shopee/营销中心/运费促销/" + oo.siteNum,
