...
Object.assign(Tool, {
    ...
    {
        a01: function (B1, arr, start_time, end_time, seller, site, num, next, This, t) {
            let oo = {
                B1: B1,
                arr: arr,
        ...
        d01: function (oo) {
            ...
            let data = [{
                ...
                sql: "select " + Tool.fieldAs("fromid,newDiscount") + " FROM @.table where @.status=1 and @.MinimumOrder<=100 and @.isSeckill=1 order by @._1688_saleNum desc limit 150",
            }]
            $("#state").html("正在获取加购商品。。。");
            Tool.ajax.a01(data, this.d02, this, oo);
        },
        d02: function (t, oo) {
            oo.shopPro = t[0];//折扣还要用一下。
            let nArr = []
            for (let i = 0; i < t[0].length; i++) {
                if (oo.arr.indexOf(t[0][i].fromid) == -1 && nArr.length < 100) {
                    nArr.push(t[0][i].fromid);
                }
            }
            let data = {
        ...
        d05: function (arr, oo) {
            ...
            if (!isErr) { this.d06(data, oo) } else { Tool.pre(["出错005", sub_item_list]); }
        },
        d06: function (data, oo) {
            ...
            $("#state").html("正在保存加购商品。。。");
            gg.postFetch(url, JSON.stringify(data), this.d07, this, oo)
        },
        d07: function (t, oo) {
            $("#state").html("保存加购出来了。");
            if (t.message == "success") {
