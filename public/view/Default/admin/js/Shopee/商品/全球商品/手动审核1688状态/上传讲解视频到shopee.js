var fun =
{
    ...
    a04: function () {
        $("#state").html("正在获得配置参数");
        let where = " where @.isup=1 and @.ManualReview_1688=1 and @.ManualReview_1688_ExplanationVideo_status>3"
        let data = [{
            action: "sqlite",
    ...
    d01: function (oo) {
        ...
        if (oo.list[0][0].ExplanationVideo) { oo.list[0][0].ExplanationVideo = "https:" + oo.list[0][0].ExplanationVideo; }
        let ExplanationVideo = oo.list[0][0].ExplanationVideo
        $("#ExplanationVideoA").html('<a href="' + ExplanationVideo + '" target="_blank">' + ExplanationVideo + '</a>')
        if (oo.ExplanationVideo || !ExplanationVideo) {
            this.f05([[]])
        }
        else {
            const video = document.createElement('video');
            video.preload = 'metadata';
            video.src = ExplanationVideo;
            ...
            video.onloadedmetadata = function () {
                if (video.duration < 10) {
                    ...
                    Tool.getFileBlob(ExplanationVideo, This.d02, This, oo)
                }
            };
        }
    },
    d02: function (blob, oo) {
        ...
        Tool.blobToArrayBuffer(blob).then(arrayBuffer => {
            let o1 = {
                ...
                "file_name": oo.list[0][0].ExplanationVideo.split("/").pop()
            }
            //后面的提交要用
            oo.post = {
                "video_info": {
    ...
    e01: function (t, oo) {
        ...
        let data = [
            { name: "file", value: "（二进制）" + oo.list[0][0].ExplanationVideo },
    ...
    e02: function (t, oo) {
        $("#state").html("上传视频成功。");
        let ExplanationVideo = "https://down-ws-global.vod.susercontent.com/" + oo.post.vid + ".ori.mp4"
        $("#ExplanationVideoB").html('<a href="' + ExplanationVideo + '" target="_blank">' + ExplanationVideo + '</a>')
    ...
    f04: function (t, oo) {
        ...
        let data = [{
            action: "sqlite",
            database: "shopee/商品/全球商品",
            sql: "update @.table set @.ExplanationVideo=" + Tool.rpsql(JSON.stringify(shopeeVideo)) + " where @.ManualReview_1688_fromid=" + oo.ManualReview_1688_fromid
        }]
    ...
    f05: function (t) {
        ...
        $("#B2,#ManualReview_1688_fromid,#ExplanationVideoA,#ExplanationVideoB,#proid").html("");
        this.a04();
    },
    g01: function (ManualReview_1688_ExplanationVideo_status, ManualReview_1688_fromid) {
        let data = [{
            action: "sqlite",
            database: "shopee/商品/全球商品",
