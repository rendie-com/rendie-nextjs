...
var fun =
{
    ...
    a01: function () {
        ...
        //o.params.site          站点
        //o.params.status        状态
        let html = Tool.header(o.params.return, "Shopee &gt; 商品列表 &gt; 店铺商品 &gt; 状态_上架") + '\
        <div class="p-2">\
            ...
            <tbody>\
		        ...
		        <tr><td class="right">第几个店铺：</td><td colspan="2">'+ o.params.num + '</td></tr></tbody>\
 		        <tr><td class="right">当前商品状态：</td><td colspan="2">'+ this.b01(o.params.status) + '</td></tr>\
    ...
    b03: function () {
        ...
        if (this.obj.mode == 1) {
            ...
            data = [{
                ...
                sql: "select @.fromid as fromid from @.table where  @.status=" + o.params.status + " and @.price_uptime<>1 order by @._1688_saleNum desc" + Tool.limit(12, this.obj.A1),
            }]
            if (this.obj.A2 == 0) {
                data.push({
                    action: "sqlite",
                    database: "shopee/商品/店铺商品/" + this.obj.siteNum,
                    sql: "select count(1) as total FROM @.table where @.status=" + o.params.status + " and @.price_uptime<>1",
                })
            }
        }
            ...
            data = [{
                action: "sqlite",
                database: "shopee/商品/店铺商品/" + this.obj.siteNum,
                sql: "select @.fromid as fromid from @.table where  @.status=" + o.params.status + " and @.price_uptime<>1 order by @._1688_saleNum desc limit 10",
            }]
        }
            ...
            data = [{
                action: "sqlite",
                database: "shopee/商品/店铺商品/" + this.obj.siteNum,
                sql: "select @.fromid as fromid from @.table where  @.status=" + o.params.status + " and @.price_uptime<>1 order by @._1688_saleNum desc limit 50",
            }]
        }
        return data;
    },
