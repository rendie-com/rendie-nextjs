...
var fun =
{
    ...
    a01: function () {
        ...
        //o.params.site          站点
        //o.params.status        状态       
        let html = Tool.header(o.params.return, "Shopee &gt; 商品列表 &gt; 店铺商品 &gt; 状态 &gt; 下架") + '\
        <div class="p-2">\
    ...
    b03: function () {
        ...
        //@.isUnlisted=1        表示【能下架】
        if (this.obj.mode == 1) {
            where = " where  @.status=" + o.params.status + " and @.isUnlisted=1"
            data = [{
                action: "sqlite",
            ...
            data = [{
                action: "sqlite",
                database: "shopee/商品/店铺商品/" + this.obj.siteNum,
                sql: "select @.fromid as fromid from @.table where  @.status=" + o.params.status + " and @.isUnlisted=1 order by @._1688_saleNum asc limit 10",
            }]
        }
