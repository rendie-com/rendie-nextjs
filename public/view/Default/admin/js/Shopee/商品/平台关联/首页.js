...
var fun =
{
    a01: function () {
        ...
        o.params.ManualReview_1688_video_status = o.params.ManualReview_1688_video_status ? o.params.ManualReview_1688_video_status : "";//人工审核1688视频状态
        //o.params.manualReview               敦煌手动审核状态
        //o.params.penalty_type               更新后违规类型
        this.a02();
    },
    a02: function () {
        Tool.download_sqlite.a01(["shopee/商品/全球商品"], this.a03, this);
    },
    a03: function (t) {
        let where = this.b05()
        let data = [{
            ...
            action: "sqlite",
            database: "shopee/商品/全球商品",
            sql: "select " + Tool.fieldAs("pic,proid,fromID,ManualReview,DHAfterReview,isup,penalty_type,ManualReview_1688_video_status,addtime,uptime") + " FROM @.table" + where + Tool.limit(10, o.params.page, "sqlite"),
        }, {
            action: o.DEFAULT_DB,
            database: "main",
    ...
    //////////////////////////////////////////////
    b01: function (t) {
        let config = JSON.parse(t[0].value)["关联平台"]
        let html = '\
        <tr>\
          <th style="padding-left:25px;position: relative;" class="w120">'+ this.b10() + '编码</th>\
          <th class="w110">首图</th>\
          <th class="p-0">'+ this.b02(o.params.manualReview, config["敦煌手动审核状态"]) + '</th>\
          <th class="p-0">'+ this.b03("", config["敦煌审核后本地状态"]) + '</th>\
          <th class="p-0">'+ this.b15("", config["更新后违规类型"]) + '</th>\
          <th class="p-0">'+ this.b17(o.params.ManualReview_1688_video_status, config["人工审核1688视频状态"]) + '</th>\
    ...
    b05: function () {
        ...
        if (o.params.penalty_type) { arr.push("@.penalty_type=" + o.params.penalty_type); }
        // if (obj.arr[12] != "-_-20") { arr.push("@.ManualReview_1688=" + obj.arr[12]); }
        // if (obj.arr[13] != "-_-20") { arr.push("@.ManualReview_1688_state=" + obj.arr[13]); }
        if (o.params.ManualReview_1688_video_status != "") { arr.push("@.ManualReview_1688_video_status=" + o.params.ManualReview_1688_video_status); }
        // let str = ""
        // switch (obj.arr[14]) {
    ...
    b10: function () {
		...
		<ul class="dropdown-menu">\
            ...
            <li onClick="Tool.openR(\'jsFile='+ o.params.jsFile + '&jsFile2=02\');">\
                <a class="dropdown-item pointer" title="手动审核通过:敦煌网 &gt; 已上传商品 &gt; 商品 &gt; 手动审核状态 &gt; 9.图片且详情审核通过">获取敦煌网【手动审核通过】的商品</a>\
            </li>\
            <li onClick="Tool.openR(\'jsFile='+ o.params.jsFile + '&jsFile2=03\');"><a class="dropdown-item pointer">*将【未上传】的商品上传到【Shopee全球商品】</a></li>\
            <li onClick="Tool.openR(\'jsFile='+ o.params.jsFile + '&jsFile2=04\');" title="当你上传商品时，出现错误，上传了，没被记录，就要用到该功能。"><a class="dropdown-item pointer">*删除【Shopee全球商品】中【没被记录】的商品</a></li>\
            <li onClick="Tool.openR(\'jsFile='+ o.params.jsFile + '&jsFile2=05\');" title="当你直接在【Shopee全球商品】删除商品时，但记录的信息还在，就要用到该功能。"><a class="dropdown-item pointer">*重新记录【已上传】的商品</a></li>\
            <li onClick="Tool.openR(\'jsFile='+ o.params.jsFile + '&jsFile2=06&table=GlobalPro&database=shopee_bak&newdatabase=shopee/商品/全球商品\');"><a class="dropdown-item pointer">把一个db文件拆分成多个db文件</a></li>\
		</ul>'
    },
    ...
    b15: function (val, GlobalPro_penalty_type_count) {
        let nArr = [], arr = Tool.penalty_type;
        for (let i = 0; i < arr.length; i++) {
            nArr.push('<option value="' + i + '" ' + ("" + arr[i][0] == "" + val ? 'selected="selected"' : '') + '>' + i + '.' + arr[i][1] + '（' + GlobalPro_penalty_type_count[i] + '）</option>');
        }
        return '\
    ...
    c02: function () {
        ...
        if (field == "2" && isNaN(searchword)) {
            ...
            Tool.main("jsFile=" + o.params.jsFile + "&jsFile2=" + o.params.jsFile2 + "&site=" + o.params.site + "&page=1&field=" + field + "&searchword=" + searchword);
        } else { alert("请输入搜索内容"); }
    },
    ...
    c04: function (val) {
        if (val == "-2") {
            Tool.open4("jsFile=" + o.params.jsFile + "&jsFile2=07");
        }
        else {
            Tool.open(8, val);
        }
    ...
    c07: function (val) {
        if (val == "-2") {
            Tool.openR("jsFile=" + o.params.jsFile + "&jsFile2=08");
        }
        else {
