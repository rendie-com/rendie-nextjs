...
var fun =
{
    ...
    a01: function () {
        ...
        let html = Tool.header(o.params.return, "Shopee &gt; 商品 &gt; 违规或删除 &gt; 更多 &gt;  获取【违规或删除】信息") + '\
        <div class="p-2">\
    ...
    e03: function (t, products) {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (this.b01(products[i].id, t[1])) {
                let reasons = products[i].banned_info.reasons[0]
                if (!reasons) { reasons = { penalty_type: 0 } }
                data.push({
                    action: "sqlite",
                    database: "shopee/商品/违规或删除",
                    sql: "insert into @.table(@.productId,@.proid,@.name,@.status,@.pic,@.addtime,@.uptime,@.banned_time,@.penalty_type,@.description,@.explanation,@.site)values(" + products[i].id + "," + proid + "," + Tool.rpsql(products[i].name) + "," + products[i].status + "," + Tool.rpsql(products[i].cover_image) + "," + products[i].create_time + "," + products[i].modify_time + "," + products[i].banned_info.banned_time + "," + reasons.penalty_type + "," + Tool.rpsql(reasons.description) + "," + Tool.rpsql(reasons.explanation) + ",'" + site + "')",
                })
            }
            else {
                data.push({
