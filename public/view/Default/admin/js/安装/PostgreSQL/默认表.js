...
let mssql = [
    ...
    //         "insert into @.sqlhelp(@.note,@.sql) values ('新建表','create table [表名]([自动编号字段] int identity(1,1) primary key ,[字段1] varchar(50) default 默认值 null ,[字段2] text null ,[字段3] datetime,[字段4] money null ,[字段5] int default 0,[字段6] Decimal (12,4) default 0,[字段7] image null)')",
    //         "insert into @.sqlhelp(@.note,@.sql) values ('删除数据','DELETE FROM [表名] WHERE [字段名]>100')",
    //         "insert into @.sqlhelp(@.note,@.sql) values ('新增字段','alter table [表名] add [字段名] varchar (50) NULL<br>alter table [表名] add [字段名] double default 0')",
    //         "insert into @.sqlhelp(@.note,@.sql) values ('删除字段','alter table [表名] DROP column [字段名]')",
    //         "insert into @.sqlhelp(@.note,@.sql) values ('修改字段','alter table [表名] alter column [字段名] varchar (50) NULL')",
    ...
    //         "insert into @.sqlhelp(@.note,@.sql) values ('添加字段1','alter table @.dhUpPro add @.upGroupId varchar(32)')",
...
//PostgreSQL 支持的字段类型         https://blog.csdn.net/yhj198927/article/details/135458630