...
var fun =
{
    ...
    sql: {},//保存到商品表的语句
    sqlDes: {},//保存到商品详情表的语句
    file: "/" + o.cacheFolder + "collect/aliexpress/" + obj.arr[4] + ".js",
    a01: function () {
        obj.arr[4] = obj.arr[4] ? obj.arr[4] : "-_-20";//ID
        obj.arr[5] = obj.arr[5] ? obj.arr[5] : "-_-20";//模式【如：1:预览；2:采集；3:更新】
        //obj.arr[4]      返回URL
        //obj.arr[5]      模式【如：1:预览；2:采集；】
        //obj.arr[6]      【采集表ID】,【allUser所有已上传店铺】,【proid商品proid】,【allshop按店铺采】【类目ID】
    ...
    a05: function () {
        ...
        let URL = this.obj.code.a.pageurl1;
        ...
        else { alert("没做：分页设置：	不分页	 批量分页	 手动分页	 直接采集内容页(分页)	 提交分页(post)"); }
    },
    a06: function () {
    ...
    a08: function (html) {
        ...
        if (str == false) {
            ...
            if (fromidArr.length == 0) {
                ...
                let txt = '[0<r:pro db="sqlite.aliexpress" where=" where @.fromid in(' + fromidArr.join(",") + ')" size="100">,"<:fromid/>"</r:pro>]'
    ...
    a09: function (oo, fromidArr) {
        ...
        for (let i = 0; i < nArr.length; i++) {
            if (this.obj.code.b.isspecialmlink == "1") { this.cache.item[i] = this.obj.code.b.mlinkRR.replace("[变量]", this.cache.item[i]) };
        }
        ///////////////////////////////////////////////
        $("#state").html('正在去掉【重复】。。。');
    ...
    a14: function ()//生成商品ID
    {
        ...
        let str = '<.Db(sqlite.aliexpress,select count(1) from @.pro where @.proid=\'R' + (ran) + '\',count)/>'
    ...
    a21: function () {
        let str = '<r: db="sqlite.aliexpress">update @.gather set @.LastTime=' + Tool.gettime("") + ' where @.id=' + obj.arr[4] + '</r:>';
    ...
    b03: function (table) {
        ...
        insert1 = '<r: db="sqlite.aliexpress">insert into @.' + table + '(' + props1.substr(1) + ')values(' + props2.substr(1) + ')</r:>';
        ///////////////////////////////////////
        props1 = ""; props2 = ""
        for (let p in sqlDes) {
            props1 += ",@." + p;
        ...
        insert2 = '<r: db="sqlite.aliexpress_prodes/' + Tool.pronum(sql.proid, 50) + '">insert into @.prodes(' + props1.substr(1) + ')values(' + props2.substr(1) + ')</r:>';
        /////////////////////////////////
        str = '<if Fun(Db(sqlite.aliexpress,' + select1 + ',count))==0>' + insert1 + insert2 + '</if>'
        return str
    }
}
fun.a01();