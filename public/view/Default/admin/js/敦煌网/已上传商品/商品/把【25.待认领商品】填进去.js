...
var fun =
{
    ...
    a01: function () {
        //obj.arr[4]    返回URL
        //obj.arr[5]    手动审核状态 （2：详情不存在）
        let html = Tool.header("把【25.待认领商品】填进去...") + '\
        <div class="p-2">\
    ...
    a04: function () {
        ...
        //proupdhgateB      表示【详情不存在】的商品
        //ManualReview=2    表示【手动审核状态】的【详情不存在】
        //proStatus=25      表示【本地商品状态】的【待认领商品】
        if (this.obj.Aarr.types) {
            let str = '{\
            "proupdhgateA":[0\
                <r:proupdhgate size=10 db="sqlite.dhgate" where=" where @.type1=' + this.obj.Aarr.types + ' and @.proStatus=25">,\
                {\
                    "fromid":<:fromid/>,\
                    "proid":"<:proid/>",\
            ...
            "proupdhgateB":[\
                ...
                <r:proupdhgate page=2 size=10 db="sqlite.dhgate" where=" where @.upuserid=' + this.obj.Aarr.fromid + ' and @.ManualReview=' + obj.arr[5] + '">,\
                {\
                    "proid":"<:proid/>",\
                    "fromid":<:fromid/>\
                }\
    ...
    a07: function (proupdhgateA, proupdhgateB) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            sqlArr2.push('update @.proupdhgate set @.pic=null,@.err=null,@.ManualReview=' + proupdhgateA[i].ManualReview + ',@.proStatus=0,@.proid=\'' + proupdhgateA[i].proid + '\' where @.fromid=' + proupdhgateB[i].fromid);
            sqlArr2.push('update @.pro set @.isUpDHgate=0 where @.proid=\'' + proupdhgateB[i].proid + '\'');//取消之前的记录，因为被【待认领商品】替换了
        }
        let str = '""<r: db="sqlite.aliexpress">' + sqlArr1.join('<1/>') + '</r:>';//先删除，因为有唯一索引的约束。
